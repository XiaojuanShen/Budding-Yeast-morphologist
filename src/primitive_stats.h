 ...
 */
	...
	class ProbabilityDistribution : public Oper2<double, INPUT_DECL> {
	public:
	...
	public:
		...
		void EMregist( C &instance, const Tuple<double, nbstates> prob);
		void EMclear();


		double EMfinit();
		Classifier<C, nbstates> EMfinit(double alpha) const;
...
LFHTEMP void GaussianDistribution<nbchannels>::setVariance(const Tuple<double, nbchannels>& _var){
		...
		Matrianglix<double , nbchannels > mt = Matrianglix<double , nbchannels>(ihvar);
...
LFHTEMP	double ClassifierV<C>::EMfinit(){
		...
		if (unknown_scope != NULL){
			...
			if (fix_unknown_dens == true){
				...
				if (relerr < log(4)) { // dont update if too much data is in unknown class
					...
					ExCo<double>::minimum(coef[0]); // minimum value for likelihood!
	...
	LFHTEMP	void ClassifierV<C>::EMaccept(bool do_accept){
		...
		int nbstates = classes.size();
		if (!do_accept){
			if (EMsclasses == NULL) exit(0); // should always accept first!
