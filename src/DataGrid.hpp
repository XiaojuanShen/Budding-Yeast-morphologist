...
#define LFHTEMP template <class C, unsigned int nbdim>
	...
	LFHTEMP DataGrid<C, nbdim>::DataGrid(const DataGrid<C,nbdim>& other){
		...
		for(ts--;ts != 0xFFFFFFFF;ts--) data[ts] = other[ts];
	}
	...
	LFHTEMP DataGrid<C, nbdim>::~DataGrid(){
	...
	LFHTEMP DataGrid<C,nbdim-1> DataGrid<C, nbdim>::makeSlice(int which_dim, int coor)const{
	...
	LFHTEMP DataGrid<C,nbdim+1> DataGrid<C, nbdim>::fromSlice(int which_dim) const{
		DataGrid<C,nbdim+1> fout;
		Tuple<unsigned int, nbdim+1> dasize;
		unsigned int i;
    ...
    LFHTEMP DataGrid<C,nbdim>& DataGrid<C, nbdim>::memmove(DataGrid<C,nbdim>& source){
        data = source.data;
        source.data = NULL;
...
for(unsigned int i = (*this).totsize()-1; i != 0xFFFFFFFF;i--) op(data[i], a_2.data[i]);
	...
	LFHTEMP
	unsigned int const * const DataGrid<C, nbdim>::getSizes() const {return(dims);}
...
//			}
	...
	LFHTEMP
	void DataGrid<C, nbdim>::setSizes(const Tuple<unsigned int,nbdim> &in_dims){
		unsigned int i;
...
//		}
	...
	void DataGrid<C, nbdim>::initeye(){
		...
		max = modulo * (max-1);
		C* cur = &((*this)[0]);
		for(i=0;i<=max;i++) *(cur++) = ((i % modulo) == 0) ? ExCo<C>::one() : ExCo<C>::zero() ;
		for(;i<tsize;i++) *(cur++) = ExCo<C>::zero() ;
		}
	/*
	LFHTEMP
	...
	LFHTEMP
	C& DataGrid<C, nbdim>::operator()(unsigned int* p ){
		int pos=0;
	...
	LFHTEMP
	C& DataGrid<C, nbdim>::operator()(int* p){
		int pos=0;
		if (nbdim>=9) for(int i = nbdim-1;i>8;i--) pos = (pos + p[i]) * dims[i-1];
		if (nbdim>=8) pos = (pos + p[7]) * dims[6];
		if (nbdim>=7) pos = (pos + p[6]) * dims[5];
	...
	LFHTEMP
	C DataGrid<C, nbdim>::operator()(unsigned int* p ) const{
		int pos=0;
		if (nbdim>=9){
			int i = nbdim-1;
			for(i=0;i>8;i--) pos = (pos + p[i]) * dims[i-1];
		}
		if (nbdim>=8) pos = (pos + p[7]) * dims[6];
		if (nbdim>=7) pos = (pos + p[6]) * dims[5];
		if (nbdim>=6) pos = (pos + p[5]) * dims[4];
	...
	LFHTEMP
	C DataGrid<C, nbdim>::operator()(int* p) const{
		int pos=0;
		if (nbdim>=9) for(int i = nbdim-1;i>8;i--) pos = (pos + p[i]) * dims[i-1];
		if (nbdim>=8) pos = (pos + p[7]) * dims[6];
		if (nbdim>=7) pos = (pos + p[6]) * dims[5];
	...
	LFHTEMP
	C& DataGrid<C, nbdim>::operator()(const Tuple<unsigned int,nbdim>& p ){
		int pos=0;
		if (nbdim>=9) for(int i = nbdim-1;i>8;i--) pos = (pos + p[i]) * dims[i-1];
		if (nbdim>=8) pos = (pos + p[7]) * dims[6];
		if (nbdim>=7) pos = (pos + p[6]) * dims[5];
	...
	LFHTEMP
	C& DataGrid<C, nbdim>::operator()(const Tuple<int,nbdim>& p){
		int pos=0;
		if (nbdim>=9) for(int i = nbdim-1;i>8;i--) pos = (pos + p[i]) * dims[i-1];
		if (nbdim>=8) pos = (pos + p[7]) * dims[6];
		if (nbdim>=7) pos = (pos + p[6]) * dims[5];
	...
	LFHTEMP
	C DataGrid<C, nbdim>::operator()(const Tuple<unsigned int,nbdim>& p ) const{
		int pos=0;
		if (nbdim>=9) for(int i = nbdim-1;i>8;i--) pos = (pos + p[i]) * dims[i-1];
		if (nbdim>=8) pos = (pos + p[7]) * dims[6];
		if (nbdim>=7) pos = (pos + p[6]) * dims[5];
	...
	LFHTEMP
	C DataGrid<C, nbdim>::operator()(const Tuple<int,nbdim>& p) const{
		int pos=0;
		if (nbdim>=9) for(int i = nbdim-1;i>8;i--) pos = (pos + p[i]) * dims[i-1];
		if (nbdim>=8) pos = (pos + p[7]) * dims[6];
		if (nbdim>=7) pos = (pos + p[6]) * dims[5];
	...
	LFHTEMP
	C DataGrid<C, nbdim>::linearInpterpolation(const Tuple<double,nbdim> & coor) const{
		C* mini = data + ((unsigned int)floor(coor[0]));
		double buf[nbdim];
		unsigned int offset[nbdim];
		C partial[nbdim];
		if (nbdim > 1) {
			offset[1] = dims[0];
			mini += offset[1] * ((unsigned int)floor(coor[1]));
			for(int i=2;i<nbdim;i++) {
				offset[i] = dims[i-1] * offset[i-1];
				mini += offset[i] * ((unsigned int)floor(coor[i]));
			}
			}

		for(unsigned int i = nbdim-1;i<16;i--) buf[i] = coor[i] - ((unsigned int)floor(coor[i]));
		int d=0;
		for(int i=0; ;i++){
			if ((i & 1) == 0) partial[0] = *(mini++);
			else {partial[0] = (1.0f - buf[0]) * partial[0] + buf[0] *(*(mini--));
				for(d=1; (d<nbdim)&&((i >> d) & 1);d++) {partial[d] = (1.0f - buf[d]) * partial[d] + buf[d] * partial[d-1]; mini -= offset[d];}
...
/*
	...
	LFHTEMP
	unsigned int DataGrid<C, nbdim>::getDimSize(unsigned int d) {return(dims[d]);}

	LFHTEMP
	void DataGrid<C, nbdim>::getDims(unsigned int* out_dim){memcpy(out_dim,dims,sizeof(unsigned int)*nbdim);}

	LFHTEMP	void DataGrid<C, nbdim>::getDims(Tuple<unsigned int, nbdim> &o_dims) const{	for(unsigned int i=0;i<nbdim;i++) o_dims[i] = dims[i];}

	LFHTEMP
	void DataGrid<C, nbdim>::makeFreqConvolutionMap(int size, double low, double high){
	...
	LFHTEMP	DataGrid<C,nbdim> DataGrid<C, nbdim>::FFtransform_old() const{
		...
		for(curdir =0;curdir<nbdim;curdir++){
			Vector<complex> bluewindow = Vector<complex>::bluesteinWindow(dims[curdir]);
			Vector<C> f_in;
	...
	//		printf("%f\n",dists2[0]);
			...
			for(altdim=0;altdim<nbdim;altdim++) {
				if (coors[altdim] == dims[altdim]-1) coors[altdim] =0;
					...
					for(altdim--;altdim>=0;altdim--) dists2[altdim] = dists2[altdim+1];
				break;
				}
			}
			if (altdim == nbdim) break;
	...
	LFHTEMP
	void DataGrid<C, nbdim>::show(FILE* f, int level) const{
		unsigned int x,y;
		if (nbdim == 1){
			for(x=0;x<dims[0];x++) {
	...
	LFHTEMP
	void DataGrid<C, nbdim>::save(FILE* f) const{
			fwrite(dims,sizeof(unsigned int), nbdim,f);
			unsigned int tsize = dims[0];
			unsigned int i;
			for(i=1;i<nbdim;i++) tsize *= dims[0];

			if (ExCo<C>::IsPOD){
				fwrite(this->data,sizeof(C), tsize,f);
			}else{
				for(i=0;i<tsize;i++) ExOp::save(this->data[i],f);
			}
	}


	LFHTEMP
	void DataGrid<C, nbdim>::load(FILE* f, unsigned int lenght){
			int x,y;
			unsigned int dbuf[nbdim];
			fread(dbuf,sizeof(unsigned int), nbdim,f);
			setSizes(dbuf);
			unsigned int tsize = dims[0];
			unsigned int i;
			for(i=1;i<nbdim;i++) tsize *= dims[0];
			if (ExCo<C>::IsPOD){
				fread(this->data,sizeof(C), tsize,f);
			}else{
				for(i=0;i<tsize;i++) ExOp::load(this->data[i],f);
			}
	}

		LFHTEMP
		DataGrid< double ,nbdim> DataGrid<C, nbdim>::ExpectedDistanceToOther(double exit_prob) const{
	...
	void DataGrid<C, nbdim>::ExpectedDistanceToOther_instate(const DataGrid< ARR_double ,nbdim> &source,const DataGrid<double,2>&  transition, int state){
		...
		for(i=0;i<nbdim;i++) {
			transit[i][0] = transition[state + j * nbstates]; // == 1.0f - exp(alpha)
			transit[i][nbdim] = log(1.0f - transition[state + j * nbstates]); // == 1.0f - exp(alpha)
			for(j=1;j<nbdim;j++) {
	...
	LFHTEMP
	void DataGrid<C, nbdim>::ExpectedDistanceToOther(const DataGrid< double ,nbdim+1> &source, const DataGrid<double,2>& transition, bool state_zero_special){
...
//				LFH_ALIVE;
		...
		LFHTEMP DataGrid<C,nbdim> & DataGrid<C, nbdim>::convolvecircle_zeroborder(double radius){
			...
			ExOp::toZero(coor); do{
				tmpd = coor[0] * coor[0];
				for(maxb=1;maxb<nbdim;maxb++) tmpd += coor[maxb] * coor[maxb];
				if (sqrt(maxb) < radius){sumd += 1.0f;dawin(coor) = 1.0f;}
				for(maxb=0;maxb<nbdim;maxb++) {
					if (coor[maxb] == ((dims[maxb]) << 1)-1) {coor[maxb] =0; vec[maxb] = 0.0f;}
					else {
						coor[maxb]++;
	...
	LFHTEMP const DataGrid<C,nbdim> & DataGrid<C, nbdim>::blur_crude(const GaussianDistribution<nbdim> &gauss){
		unsigned int coor[nbdim];
		unsigned int dir,altdir;
				...
				if (doit){
					...
					switch(tmpcmp & SETCMP_CMP_MASK){
						case SETCMP_EQUAL:
							...
							break;
						case SETCMP_MASKED_GE:
							_out(coor)++;
							break;
						case SETCMP_MASKED_LE:
							_out(altcoor)++;
							break;
						...
						do {
							...
							switch(tmpcmp & SETCMP_CMP_MASK){
								...
								case SETCMP_MASKED_LE:
									...
									break;
								default:
									printf("no!\%in", tmpcmp & SETCMP_CMP_MASK);
							}
...
LFHTEMP DataGrid<unsigned int,nbdim> DataGrid<C, nbdim>::makeWatershedSortIndex(unsigned int &nbpix,  bool min_gradient_bassin, bool tree_skip_save) const{ // no_cheese inclues lone surrounded pixels
  ...
  //  bool has_cheese_up = false;
    ...
    if (min_gradient_bassin){
        printf("Permorming Watershed transform:"); fflush(stdout);
        DataGrid<double , nbdim> gradient = this->makeGradientNorm();
        fout = gradient.SegementClimbToMin(tsize);
        
        printf(" DONE\nPerparing Probabilistic clustering:"); fflush(stdout);
        ...
        for(i=0;i<tsize;i++){
            ...
            for(j=0;j<exist.heap.getSize();j++){
                toins.d = exist.heap[j].first.k; // printf("%i\n", toins.d.second); 
                if (toins.d < i) {
	...
	while(!merges.isempty()){
        ...
        if (!areas.isempty()) {
            ...
            stat[i][0] = toins.d;
            loc_merges[ stat[stat[i][0]][2] ].clear();
	...
	//	printf("%i\n", merges.getSize());
        ...
        stat[i][0] = toins.d;
        if (loc_merges[l].isempty()) {fprintf(stderr,"empty?\n");exit(1);}
     ...
     //   printf("alive Y! %i\n", toins.d); fflush(stdout);
      ...
      //     printf("alive T!\n"); fflush(stdout);
		...
		if ((i % 500)==0) {
        printf("%i out of %i,   (merge dist = %e) \n",i-tsize, tsize -1, toins.k); fflush(stdout);
        }
  ...
  //      printf("aliveb! %i\n",stat[stat[i][1]][2]); fflush(stdout);
        ...
        loc_merges[stat[i][2]].clear();
        loc_merges[stat[stat[i][1]][2]].clear();


		// MARK AS DONE!
        if (stat[i][0]  == stat[i][1]) {fprintf(stderr,"got equal...\n");exit(1);}
...
LFHTEMP void DataGrid<C, nbdim>::drawLine(const Tuple<unsigned int,nbdim> &a, const Tuple<unsigned int,nbdim> &b, const C& value){
	unsigned int i,j,be,t;
	i=0;be = a[0] >= b[0] ? a[0] - b[0] : b[0] - a[0];
	for(j=1;j<nbdim;j++) {t = a[i] >= b[i] ? a[i] - b[i] : b[i] - a[i]; if (t > be) {be=t;i=j;};}
	Tuple<unsigned int, nbdim> coor = a;
	Tuple<unsigned int, nbdim> res; ExOp::toZero(res);
	(*this)(coor) = value;
	for(t=0;t<be;t++){
		for(i=0;i<nbdim;i++) if (a[i] < b[i]) { 
			if ( (res[i] += (b[i] - a[i])) >= b ) {res[i] -=b; coor[i]++;}
			}else if ( (res[i] += (a[i] - b[i])) >= b ) {res[i] -=b; coor[i]--;}
...
LFHTEMP template<unsigned int size> void DataGrid<C, nbdim>::drawDownChar(unsigned char which, const Tuple<unsigned int,nbdim-1> &where, const Tuple<C,size> &what){
	...
	/*
template <class D, int nbdim, bool pow2> template<int opsize>
int DataGrid<D, nbdim, pow2>::operator()(GridOperation<opsize> const & op){

	switch(op.type){
		case LFHOPERATOR_GRID_EXPONENTIALBLUR:
			exponentialBlur(opsize == sizeof(double) ? ((double*)op.arg)[0]: 1.0f, false);
		break;
		case LFHOPERATOR_GRID_EXPONENTIALBLUR_CIRCULAR:
			exponentialBlur(opsize == sizeof(double) ? ((double*)op.arg)[0]: 1.0f, true);
		break;
		case LFHOPERATOR_GRID_HMM:
		break;
		}
	...
	template <class D, int nbdim, bool pow2>
	void DataGrid<D, nbdim, pow2>::importTiff(char* path){


	}
	template <class D, int nbdim, bool pow2>
	void DataGrid<D, nbdim, pow2>::exportTiff(char* path){
